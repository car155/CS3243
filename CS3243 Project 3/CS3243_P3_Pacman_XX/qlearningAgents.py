...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        newValue = self.getQValue(state, action) * (1 - self.alpha) + self.alpha *(reward + self.discount * self.getValue(nextState))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        diff = reward - self.getQValue(state, action) + (self.getValue(nextState) * self.discount)
        features = self.featExtractor.getFeatures(state, action)
        
        for f in features.keys():
          features[f] *= (self.alpha * diff)
