...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            position, curr_state = state
            x, y = position
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            visted_corners = list(curr_state)

            if (not self.walls[nextx][nexty]):
                if ((nextx, nexty) in self.corners):
                    idx = self.corners.index((nextx, nexty))
                    visted_corners[idx] = 1
                nextState = ((nextx, nexty), visted_corners)
...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(visited_corners)):
        if visited_corners[i] != 1:
            ...
            dist = abs(nextx - goalx) + abs(nexty - goaly)
